...
-- 고용형태
DROP TABLE IF EXISTS edu_employtype RESTRICT;
...
CREATE TABLE edu_lect (
  ...
  mmo     INTEGER      NULL     COMMENT '매니저번호' -- 매니저번호
)
COMMENT '강의';
...
CREATE TABLE edu_student (
  ...
  COL22     VARCHAR(50)  NULL     COMMENT '최종학교', -- 최종학교
  COL21     VARCHAR(50)  NULL     COMMENT '전공', -- 전공
  bno       INTEGER      NOT NULL COMMENT '은행번호', -- 은행번호
  COL24     VARCHAR(20)  NOT NULL COMMENT '계좌번호', -- 계좌번호
  COL25     BOOLEAN      NOT NULL COMMENT '재직여부' -- 재직여부
)
COMMENT '학생';
...
CREATE TABLE edu_manager (
  mmo  INTEGER     NOT NULL COMMENT '매니저번호', -- 매니저번호
...
-- 고용형태
ALTER TABLE edu_employtype
  ADD CONSTRAINT PK_edu_employtype -- 고용형태 기본키
  PRIMARY KEY (
  etno -- 고용형태번호
  );
...
-- 강사
ALTER TABLE edu_teacher
  ADD CONSTRAINT FK_edu_employtype_TO_edu_teacher -- 고용형태 -> 강사
  FOREIGN KEY (
  etno -- 고용형태번호
  )
  REFERENCES edu_employtype ( -- 고용형태
  etno -- 고용형태번호
  );
